...
class LMModel(nn.Module):
    ...
    def __init__(self, args, dictionary):
        ...
        self.decoder = TransformerDecoder(self.config, embed_tokens)
        self.out_proj = nn.Linear(args.embedding_dim, len(dictionary))
    
    def get_hidden(self, source):
        decoder_input_ids, decoder_padding_mask, causal_mask = _prepare_decoder_inputs(self.config, None, source)
        encoder_hidden_states = torch.zeros(source.shape[0], 1, self.config.n_embed, device=source.device)
        hidden = self.decoder(decoder_input_ids, encoder_hidden_states, None, decoder_padding_mask, causal_mask)
        return hidden

    def logits(self, source, **unused):
        hidden = self.get_hidden(source)
    ...
    def generate(self, prefix, max_len=100, beam_size=None):
        ...
        '''
        with torch.no_grad():
            device = list(self.out_proj.parameters())[0].device #wtf
            source = torch.LongTensor(1, len(prefix) + 1)
            source[0, 0] = self.dictionary.bos()
            source[0, 1:] = self.dictionary.encode_line(prefix, append_eos=False)
            ...
            for _ in range(max_len - len(prefix)):
                ...
                for pos in range(head, tail):
                    x, prob = bfs_path[pos]
                    y = self.get_hidden(x) # (bsz, len, embed_dim)
                    probs.append(prob * F.softmax(self.out_proj(y)[0, -1], dim=0))
                probs_tensor = torch.cat(probs)
                values, indices = probs_tensor.topk(beam_size)
                values /= values.sum()
                for value, index in zip(values, indices):
                    prob = value.item()
                    pre = index.item() // self.config.vocab_size
                    idx = index.item() % self.config.vocab_size
                    if idx == self.dictionary.eos():
                        head = head + pre
                        tail = head + 1
                        flag = True
                        break
                    x = torch.LongTensor(1, 1).to(device)
                    x[0, 0] = idx
                    x = torch.cat([bfs_path[head + pre][0], x], dim=1)
                    bfs_path.append((x, prob))
                if flag == True:
                    break
                head = tail
                tail = len(bfs_path)
        max_pos = np.argmax(np.array([bfs_path[pos][1] for pos in range(head, tail)])) + head
        outputs = ""
        for idx in bfs_path[max_pos][0][0, 1:]:
            outputs += self.dictionary[idx.item()]
        return outputs
...
class Seq2SeqModel(nn.Module):
    ...
    def generate(self, inputs, max_len=100, beam_size=None):
        ...
        output 下联: "复兴政策暖万家"
        '''
        # max_len = min(max_len, len(inputs))
        with torch.no_grad():
            device = list(self.out_proj.parameters())[0].device #wtf
            source = torch.LongTensor(1, len(inputs) + 1)
            source[0, 0] = self.dictionary.bos()
            source[0, 1:] = self.dictionary.encode_line(inputs, append_eos=False)
            source = source.to(device)
            bfs_path = [(source[:, :1], 1)]
            head = 0
            tail = 1
            for _ in range(min(max_len, len(inputs))):
                probs = []
                flag = False
                for pos in range(head, tail):
                    x, prob = bfs_path[pos]
                    y = self.endecoder(source, x) # (bsz, len, embed_dim)
                    probs.append(prob * F.softmax(self.out_proj(y)[0, -1], dim=0))
                probs_tensor = torch.cat(probs)
                values, indices = probs_tensor.topk(beam_size)
                values /= values.sum()
...
class TransformerEncoderBackbone(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.static_position_embeddings:
            self.embed_positions = SinusoidalPositionalEmbedding(
                config.max_position_embeddings, config.n_embed, self.padding_idx
            )
        else:
    ...
    def forward(
        self, input_embeds, attention_mask=None):
        # check attention mask and invert

        scaled_inputs_embeds = input_embeds * self.embed_scale
        embed_pos = self.embed_positions(input_embeds[:, :, 0])
        x = scaled_inputs_embeds + embed_pos
        ...
        for i in range(len(self.layers)):
            ...
            if self.training and (dropout_probability < self.layerdrop):  # skip the layer
                ...
                x, attn_map = encoder_layer(x, attention_mask)
...
class Attention(nn.Module):
    ...
    def forward(
        ...
        src_len, bsz, _ = key.shape # (src_len, bsz, embed_dim)
        tgt_len, _, _ = query.shape # (tgt_len, bsz, embed_dim)
        k = self._shape(self.k_proj(key), src_len, bsz) # (bsz * num_heads, src_len, head_dim)
        v = self._shape(self.v_proj(key), src_len, bsz) # (bsz * num_heads, src_len, head_dim)
        q = self._shape(self.q_proj(query), tgt_len, bsz) # (bsz * num_heads, tgt_len, head_dim)
        attn_weight = torch.bmm(q, k.transpose(1, 2)) * self.scaling # (bsz * num_heads, tgt_len, src_len)
        if key_padding_mask is not None:
            attn_weight = attn_weight.view(bsz, self.num_heads, tgt_len, src_len)
            attn_weight = attn_weight.masked_fill(key_padding_mask.unsqueeze(1).unsqueeze(1), float('-inf'))
            attn_weight = attn_weight.view(bsz * self.num_heads, tgt_len, src_len)
        if attn_mask is not None:
            attn_weight += attn_mask
        attn_weight = F.softmax(attn_weight, dim=-1)
        attn_weight = F.dropout(attn_weight, self.dropout, self.training)
        attn_output = torch.bmm(attn_weight, v) # (bsz * num_heads, tgt_len, head_dim)
        attn_output = attn_output.transpose(0, 1).contiguous().view(tgt_len, bsz, self.embed_dim) # (tgt_len, bsz, embed_dim)
