...
# Param_Dict = {'torso':1.0,'up':0.3,'feet':0.2,'tau':0.05,'done':1,'velx':0,'badfoot':0.1,'footcontact':0.1}
Param_Dict = {}
...
class ObservationWrapper(gym.Wrapper):
    def __init__(self, env,ETG,sensor_mode,normal,ETG_H):
        ...
        self.ETG_H = ETG_H
        self.ETG = ETG
        self.ETG_mean = np.array([2.1505982e-02, 3.6674485e-02, -6.0444288e-02, 2.4625482e-02, 1.5869144e-02, -3.2513142e-02, 
                                  2.1506395e-02, 3.1869926e-02, -6.0140789e-02, 2.4625063e-02, 1.1628972e-02, -3.2163858e-02])
...
class ETGWrapper(gym.Wrapper):
    def __init__(self, env,ETG,ETG_T,ETG_path,ETG_T2,ETG_H=20,act_mode="traj",task_mode="normal",step_y=0.05):
        ...
        if self.ETG:
            self.ETG_agent = ETG_layer(self.ETG_T,self.env.env_time_step,self.ETG_H,0.026,phase,0.2,self.ETG_T2)
            self.ETG_weight = 1
...
class RewardShaping(gym.Wrapper):
    ...
    def terminate(self,info):
        ...
        # return False
        return rot_mat[-1]<0.5  or np.mean(footz)>-0.1 or np.max(footz)>0  or (base_std<=2e-4 and self.steps>=10) or abs(pose[-1])>1.5
