...
def pillar_terrain(length=0.1, width=0.1,height=1.0,stepnum=60):
    """By ZKF."""
    add_height = 0
    env_info = []
    boxHalfLength = length
    boxHalfWidth = width
    boxHalfHeight = height
    
    ground = p.createCollisionShape(p.GEOM_BOX,halfExtents=[100, 10, 0.1])
    id = p.createMultiBody(baseMass=0,
                           baseCollisionShapeIndex=ground,
                           basePosition=[40, 0, -0.09],
                           baseOrientation=[0.0, 0.0, 0.0, 1])
    p.changeDynamics(id, -1, lateralFriction=FRICTION)

    sh_colBox = p.createCollisionShape(p.GEOM_BOX,halfExtents=[boxHalfLength,boxHalfWidth,boxHalfHeight])
    # vis_colBox = p.createVisualShape(
    #     p.GEOM_BOX, 
    #     halfExtents=[boxHalfLength,boxHalfWidth,boxHalfHeight], 
    #     meshScale=[1, 1, 1],
    #     rgbaColor=[0.2, 0.5, 0, 1],
    # )
    for i in range(stepnum):
        x_pos = 1.0 + 20 * np.random.uniform(0, 1)
        y_pos = 5 * np.random.uniform(-1, 1)
        id = p.createMultiBody(baseMass=0,
                               baseCollisionShapeIndex=sh_colBox,
                               basePosition=[x_pos, y_pos, height],
                               baseOrientation=[0.0, 0.0, 0.0, 1])
        p.changeDynamics(id, -1, lateralFriction=FRICTION)
...
def mountain_terrain():
    ...
    mountain_shape = p.createVisualShape(
        p.GEOM_MESH, 
        fileName='/home/class90_2019012383/Project/robotic_course_project/mountain_1.obj', 
        meshScale=[1, 1, 1], 
        rgbaColor=[0.2, 0.5, 0, 1],
    )
    mountain_body = p.createCollisionShape(
        p.GEOM_MESH, 
        fileName='/home/class90_2019012383/Project/robotic_course_project/mountain_1.obj', 
        meshScale=[1, 1, 1],
        flags=p.GEOM_FORCE_CONCAVE_TRIMESH,
    )
    # quaterion = QuaternionFromAxisAngle(axis=[1,0,0],angle=np.pi/2)
    id = p.createMultiBody(
        baseMass=0,
        baseCollisionShapeIndex=mountain_body,
        baseVisualShapeIndex=mountain_shape,
        basePosition=[0, 0, 1],
        baseOrientation=[0, 0, 0, 1]
    )
    p.changeDynamics(id, -1, lateralFriction=FRICTION)

    # box = p.createCollisionShape(p.GEOM_BOX,halfExtents=[0.1,0.1,1.0])
    # id = p.createMultiBody(
    #     baseMass=0,
