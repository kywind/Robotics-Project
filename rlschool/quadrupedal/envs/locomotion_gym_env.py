...
from rlschool.quadrupedal.envs.utilities.terrain import mountain_terrain, pillar_terrain, upstair_terrain, downstair_terrain,upslope_terrain
...
class LocomotionGymEnv(gym.Env):
  ...
  def _build_action_space(self):
    ...
    if motor_mode == robot_config.MotorControlMode.HYBRID:
      ...
      self.action_space = spaces.Box(np.array(action_lower_bound, dtype=np.float32),
                                     np.array(action_upper_bound, dtype=np.float32),
                                     dtype=np.float32)
  ...
  def reset(self,**kwargs):
    ...
    if self.first_reset:
      ...
      elif self.task_mode == "pillar":
        self.add_height,self.env_info = pillar_terrain()
      elif self.task_mode == "mountain":
        self.add_height,self.env_info = mountain_terrain()
